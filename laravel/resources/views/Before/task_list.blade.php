                                                ...
                                                <tbody id="zxc">
                                                    @foreach($result as $v)
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <button class="btn btn-inverse btn-xs m-b-5" onclick="location.href='{{url('Before/taskdelete')}}?id={{$v['id']}}'" >删除任务</button>
																<?php if($v['taskStatus'] == '已完成'){ ?>
                                                                <button class="md-trigger btn  btn-info btn-xs m-b-5" onclick="location.href='{{url('Before/taskdetails')}}?id={{$v['id']}}'" >查看详细</button>
																<?php } ?>
                                                            </td>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<div class="modal-content">
        ...
        <div class="modal-body">
			...
			<form role="form" action="{{url('Before/exceldemo')}}" method="post">
			...
			<form role="form" action="{{url('Before/taskjob')}}" method="post" enctype="multipart/form-data">
				<input type="hidden" name="_token" value="{{csrf_token()}}"/>
