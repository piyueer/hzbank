...
use App\Jobs\TaskAfter;
use App\Libs\Myclass\GetSingleResultAfter;
...
class AfterController extends Controller
{
	...
	public function __construct()
	{
		parent::__construct();
		$this->check_login();
		view()->share('data', 'After');
	}

	public function getSingleindex()
	{
		return view('After.after_single');
	}

	public function getTasklist()
	{		
		$result = DB::table('task_list_after as t')
		...
		return view('After.task_list')->with('result', $result);
	}
	...
	{   
		...
		$taskId = DB::table('task_list_after')->insertGetId($insertResult);
		
		$job = (new TaskAfter($excelName,$taskId))->delay(4);
        $this->dispatch($job);
		
		return redirect('After/tasklist');							
	}
	...
	{   
		...
		return view('After.task_details')->with('taskId', $taskId);
	}
	
	public function anyDataTaskAfter()
	{   
		$taskId = $_GET['taskId'];
	...
	{  
		...
		$res = DB::table('query_history_after as q')
						->join('user_after as u','u.id','=','q.uid')
						->select('u.*')
						->where('q.uid','=',$uid)
						->first();
		...
		$result = GetSingleResultAfter::getSingleResult($res['customerName'],$res['cardID'],$res['phoneNumber'],$res['authcode'],true);
		
		return view('After.after_single_result')->with('result', $result);
	}

	public function postSingleresult()
	{   
		$result = GetSingleResultAfter::getSingleResult($_REQUEST['customerName'],$_REQUEST['cardID'],$_REQUEST['phoneNumber'],$_REQUEST['authcode'],true);
		//print_r($result);die();
		return view('After.after_single_result')->with('result', $result); 
	}
