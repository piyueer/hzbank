...
class TaskAfter extends Job implements ShouldQueue
{
    ...
    public function handle()
    {
		...
		foreach($results[0] as $insertResult){
			if(!empty($insertResult['customername']) && !empty($insertResult['cardid']) && !empty($insertResult['phonenumber']) && !empty($insertResult['authcode'])){
				GetSingleResultAfter::getSingleResult($insertResult['customername'],$insertResult['cardid'],$insertResult['phonenumber'],$insertResult['authcode'],false,$this->taskId);
			}
			else
			{
				continue;
